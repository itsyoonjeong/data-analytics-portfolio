-- Step 1: Compare post-treatment login engagement across experiment groups
...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      GROUP BY 1, 2, 3, 4, 5 
      -- 1.1 Calculate the number of logins per user after treatment
      ) a
    GROUP BY 1, 2 
...
-- Step 2: Compare post-treatment days engaged across experiment groups
...
-- Step 3: Chech how new users are distributed across experiment groups
...
-- Step 4: Compare post-treatment messages sent by existing users across experiment groups
  ...
  FROM (
    ...
    FROM (
      SELECT ex.experiment,
             ...
             -- Calculate the number of messages sent per user after treatment
      FROM (
        SELECT user_id,
               experiment,
               experiment_group,
